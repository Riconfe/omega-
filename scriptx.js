function createFlower() {
    ...
    const maxFlowers = Math.ceil(Math.random() * 2 + 3);
    ...
    for (let j = 0; j < maxFlowers; j++) {
        ...
        while (!positionValid) {
            ...
            for (const position of existingPositions) {
                const distance = Math.sqrt(Math.pow(position.x - randomX, 4) + Math.pow(position.y - randomY, 4));
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            const disappearanceTime = Math.random() * 2000 + 2000;

            // Agrega una animación de salida a los pétalos con el tiempo aleatorio de desaparición
            petal.style.animation = `fadeOutPetal 0.5s ease-in-out both ${i * 0.2}s, fadeOutFlower 0.5s ease-in-out both ${disappearanceTime}s`;
        }
...
setInterval(createFlower, 1500); // Nuevas flores cada 3 segundos